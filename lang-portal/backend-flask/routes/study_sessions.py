...
def load(app):
  ...
  def create_study_session():
    try:
        ...
        user_id = g.user['id'] if hasattr(g, 'user') and g.user else 1  # Default to user_id 1 if no user is authenticated
        
        # Create a new study session
        cursor.execute('''
        INSERT INTO study_sessions (group_id, study_activity_id, created_at)
        VALUES (?, ?, ?)
        ''', (group_id, activity_id, start_time))
        session_id = cursor.lastrowid
        
        # Insert review items
        for item in review_items:
            # Map item_id to word_id and convert status string to boolean
            word_id = item.get('item_id')
            ...
            VALUES (?, ?, ?, ?)
            ''', (word_id, session_id, is_correct, datetime.now()))
        
        app.db.commit()
        return jsonify({"message": "Study session created successfully", "session_id": session_id}), 200
  ...
  def review_study_session(id):
    try:
      ...
      for item in review_items:
        ...
        INSERT INTO word_review_items (word_id, study_session_id, correct, created_at)
        VALUES (?, ?, ?, ?)
        ''', (item['word_id'], id, item['correct'], datetime.now()))
      
      app.db.commit()
      
      return jsonify({"message": "Review items added successfully"}), 201
    except Exception as e:
      return jsonify({"error": str(e)}), 500
